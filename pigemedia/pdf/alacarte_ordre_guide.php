...
if(isset($_SERVER["HTTP_HOST"])){
	...
	if(!isset($_GET["j"])) { 	
		$wikipediaURL = $url . 'index.php/alacarte/apercuOrdreGuide/' . $_GET["i"] ."/". "X" . '/pdf';
	} else {
		$wikipediaURL = $url . 'index.php/alacarte/apercuOrdreGuide/' . $_GET["i"] ."/". $_GET["j"] . '/pdf';
	}	
	$ch = curl_init();
...
$content = '<page backleft="5mm" backright="5mm" backtop="5mm" backbottom="10mm">' . $content . '</page>';
	// echo $content;
	...
	$reponse->closeCursor();
	$html2pdf->Output('Ordre_de_mission-'.$dossier."-".$daty_androany.'.pdf');
}
?>