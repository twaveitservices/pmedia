...
if(isset($_SERVER["HTTP_HOST"])){
	...
	if(!isset($_GET["j"])) { 	
		$wikipediaURL = $url . 'index.php/alacarte/apercuRecapGuide/' . $_GET["i"] ."/". "X" . '/pdf';
	} else {
		$wikipediaURL = $url . 'index.php/alacarte/apercuRecapGuide/' . $_GET["i"] ."/". $_GET["j"] . '/pdf';
	}	
	$ch = curl_init();
	...
	$reponse->closeCursor();
	$html2pdf->Output('Recap-Guide-'.$dossier."-".$daty_androany.'.pdf');
}
?>