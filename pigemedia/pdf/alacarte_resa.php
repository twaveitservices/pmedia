...
if(isset($_SERVER["HTTP_HOST"])){
	...
	if(!isset($_GET["j"])) { 	
		$wikipediaURL = $url . 'index.php/alacarte/apercuresa/' . $_GET["i"] ."/". "X" . '/pdf';
	} else {
		$wikipediaURL = $url . 'index.php/alacarte/apercuresa/' . $_GET["i"] ."/". $_GET["j"] . '/pdf';
	}	
	$ch = curl_init();
	...
	$content = '<page backleft="1mm" backright="1mm" backtop="1mm" backbottom="5mm" orientation="landscape"><page_footer><p style="font-size:11px;font-style:italic; text-align: right;font-weight:bold;">Page [[page_cu]]/[[page_nb]]</p></page_footer>' . $content . '</page>';
	// echo $content;
	...
	$reponse->closeCursor();
	$html2pdf->Output('Resa-'.$dossier."-".$daty_androany.'.pdf');
}
?>