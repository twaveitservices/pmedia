...
if(isset($_SERVER["HTTP_HOST"])){
	...
	if(!isset($_GET["j"])) { 	
		$wikipediaURL = $url . 'index.php/alacarte/apercuRoadBook/' . $_GET["i"] ."/". "X" . '/pdf';
	} else {
		$wikipediaURL = $url . 'index.php/alacarte/apercuRoadBook/' . $_GET["i"] ."/". $_GET["j"] . '/pdf';
	}	
	$ch = curl_init();
	...
	$content = '<page backleft="5mm" backright="5mm" backtop="5mm" backbottom="5mm"><page_footer><p style="font-size:11px;font-style:italic; text-align: right;font-weight:bold;">Page [[page_cu]]/[[page_nb]]</p></page_footer>' . $content . '</page>';
	// echo $content;
	// error_log("TEXTE = " . $content);
	...
	$reponse->closeCursor();
	$html2pdf->Output('Prog_guide-'.$dossier."-".$daty_androany.'.pdf');
}
?>